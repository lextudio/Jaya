...
namespace Jaya.Ui.Services
{
    public sealed class SharedService : IService, System.ComponentModel.INotifyPropertyChanged
    {
        ...
        {
            // Diagnostic: log caller info to help trace redundant saves
            var caller = "unknown";
            try
            {
            ...
            Logger.Debug("SaveConfigurations called by {Caller}", caller);
            ...
            Logger.Debug("Saved configuration to disk: DetailsViewSortSettings count={Count}",
                ApplicationConfiguration?.DetailsViewSortSettings?.Count ?? 0);
        }
